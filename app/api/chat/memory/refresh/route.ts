...
export async function POST(req: Request) {
  try {
    ...
    const baseGuidance = [
      ...
      "- 키·체중·체형·외모를 요약할 때는 반드시 같은 문장에 그 속성 소유자의 고유명사를 쓴다. 현재 화자·장면 초점·인물별 기억 대상에게 다른 참여자의 신체를 옮기지 않는다.",
      "- 인물별 사건 기억의 대상은 관계·감정 연속성을 보존할 기억 소유자이지, 사건 문장 속 모든 신체 속성의 소유자가 아니다.",
    ]
      .filter(Boolean)
      .join("\n");
