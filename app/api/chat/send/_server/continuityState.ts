...
// (2026-08) 강제 연행/압송 표현 추가.
// 기존 패턴은 "체포/구속/구금/수감/감금/억류" 같은 법률 용어만 잡아서, 소설체로 흔한
// "헌병들이 ~의 목덜미를 낚아채어 끌고 가기 시작했다" 같은 퇴장을 전혀 감지하지 못했다.
// 그 결과 연행된 인물이 상태 장부에 남지 않아 몇 턴 뒤 아무렇지 않게 재등장했다(실사고).
// 조사 뒤 서술이 길어질 수 있어 앞쪽 여유(70~90자)를 두되, 이름과 같은 문장 안으로 제한한다.
function hasDetainedStart(segment: string, rawName: string) {
  ...
  return new RegExp(
    `(?:${name}(?:은|는|이|가|도)?[^.!?]{0,70}(?:체포되|구속되|구금되|수감되|감금되|억류되|구속\\s*중|구금\\s*중|수감\\s*중)` +
      ...
      // 강제 연행/압송 — 서사체 표현
      `|${name}(?:은|는|이|가|을|를|의)?[^.!?]{0,90}(?:연행되|연행됐|연행했|연행해|압송되|압송했|호송되|호송했)` +
      `|${name}(?:은|는|이|가|을|를|의)?[^.!?]{0,90}(?:끌려갔|끌려 ?나갔|끌려 ?나가|끌려가기|끌어냈|끌어내졌)` +
      `|${name}(?:은|는|이|가|을|를|의)?[^.!?]{0,90}끌고 ?(?:갔|가기|가면서|나갔)` +
      `|(?:연행된|압송된|호송된|끌려간)\\s*${name})`,
    "u"
  ).test(segment);
}
