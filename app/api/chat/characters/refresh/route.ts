...
export async function POST(req: Request) {
  try {
    ...
    const system = [
      ...
      "For every title or kinship vocative, preserve the direction as SPEAKER -> ADDRESSEE -> TITLE. Never reverse who called whom.",
      "A Korean direct address such as '선배님, 말씀하세요' means the current speaker calls the addressee 선배님; it does not mean the addressee calls the speaker 선배님.",
      "A coerced, joking, or cover-story kinship title is only a form of address and does not establish real blood or marital kinship.",
    ...
    const user = [
      ...
      `- If summary/evidence mentions a title, write it unambiguously as '${personaName}이(가) CHARACTER를 TITLE이라고 불렀어' or 'CHARACTER가 ${personaName}을(를) TITLE이라고 불렀어'. Include both names and never swap speaker/addressee.`,
