...
test("event-only preset policy overrides generic persistent meta detection", () => {
  const route = read("app/api/chat/send/route.ts");
  const presetList = read("app/api/preset/list/route.ts");
  ...
  assert.match(presetList, /AS supportsAbilityView/u);
  assert.match(chatArea, /selectedPreset\?\.supportsAbilityView/u);
