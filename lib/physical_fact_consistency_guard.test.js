...
test("does not invent an owner for an unnamed qualitative build descriptor", () => {
  const source = "육중한 체구가 소파 깊숙이 가라앉았다. 거구의 남자가 몸을 돌렸다.";
  const result = enforcePhysicalFactOwnership({
    text: source,
    identities,
  });
...
test("does not bind coordinated and adverbial wording to the canonical owner", () => {
  const source = "육중하고 둔중한 움직임이 이어졌다. 육중하게 몸을 돌렸다.";
  const result = enforcePhysicalFactOwnership({
    text: source,
    identities,
  });
...
test("never turns a detective's action or an inanimate door into the persona's property", () => {
  const source =
    "젊은 경찰이 달려들었다. 독방의 육중한 철문이 열렸다. 바닥을 울리는 육중한 발소리가 가까워졌다.";
  const result = enforcePhysicalFactOwnership({ text: source, identities });

  assert.equal(result.qualified, 0);
  assert.equal(result.removed, 0);
  assert.equal(result.text, source);
  assert.doesNotMatch(result.text, /이춘복/u);
});
...
test("ownership prompt distinguishes memory owner from attribute owner", () => {
  ...
  assert.match(block, /형용사만 보고.*이름을.*끼워 넣지 않는다/u);
});
