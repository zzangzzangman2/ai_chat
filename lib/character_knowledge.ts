...
const CONFIDENTIAL_RELATIONSHIP_PATTERN =
  /(?:가해|범인|진범|원흉|범죄|범행|배후|비밀|은폐|숨겨|몰래|잠입|침입|스토킹|불법\s*촬영|살인|살해|독살|폭행|습격|테러|납치|유괴|강간|성폭|협박|사기|기만|배신|내통|스파이|위장|변장|복면|조종|공모|누명|알리바이|도청)/u;
...
export function isSensitiveRelationshipKnowledge(value: unknown) {
  return SENSITIVE_RELATIONSHIP_PATTERN.test(String(value || ""));
}

export function isConfidentialRelationshipKnowledge(value: unknown) {
  return CONFIDENTIAL_RELATIONSHIP_PATTERN.test(String(value || ""));
}
...
export function inferRelationshipKnownByNames(input: RelationshipKnowledgeInput) {
  ...
  if (
    relation &&
    !isSensitiveRelationshipKnowledge(`${relation} ${details}`) &&
