...
test("confidential facts are narrower than ordinary investigation context", () => {
  assert.equal(
    isConfidentialRelationshipKnowledge("감시 형사와 사건 당사자"),
    false
  );
  assert.equal(
    isConfidentialRelationshipKnowledge("몰래 침입한 진범의 비밀"),
    true
  );
});
