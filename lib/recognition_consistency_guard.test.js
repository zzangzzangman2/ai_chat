...
test("removes the entire anonymous-exit paragraph but preserves the valid roster afterward", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      {
        role: "assistant",
...
test("removes a named invented exit without truncating later valid prose", () => {
  const present = deriveCurrentScenePresence({
    messages: [
...
test("removes an invented incapacitation paragraph and preserves later valid prose", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      { role: "assistant", content: "*박지아와 소은은 함께 거실에 남아 있었다.*" },
    ],
