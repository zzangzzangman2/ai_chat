...
test("catches an unnamed current character being made to vanish from a referenced pair", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      { role: "assistant", content: "*박지아는 소은의 곁에 엎드린 채 말했다.*" },
    ],
    identities: [{ name: "박지아", aliases: ["지아"] }],
  });
  const contradiction = findScenePresenceContradiction({
    text: [
      "곁에 엎드려 있던 작은 형체는 공포를 이기지 못하고 거실 안쪽으로 도망치듯 자취를 감췄다.",
      "홀로 남겨진 소은의 어깨가 떨렸다.",
    ].join("\n\n"),
    currentUserText: "둘 다 내 여자친구 할래? 대답.",
    presentCharacters: present,
  });

  assert.equal(contradiction?.characterName, "박지아");
  assert.equal(contradiction?.kind, "unauthorized_exit");
  assert.match(contradiction?.matchedText || "", /자취를 감췄/u);
});

test("catches a relational unnamed exit even when the user did not say both", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      {
        role: "assistant",
        content: "*박지아와 소은은 나란히 거실에 앉아 서로를 바라봤다.*",
      },
    ],
    identities: [
      { name: "박지아", aliases: ["지아"] },
  ...
  const text = [
    "허공을 가르는 요구가 거실에 울려 퍼지자, 곁에서 숨을 죽이고 있던 다른 형체는 짓눌리는 위압감을 견디지 못하고 슬금슬금 바닥으로 기어 내려가 짙은 그림자 속으로 자취를 감췄다.",
    "순식간에 의자 위에는 오직 소은만이 홀로 남겨졌다.",
    "잠시 뒤 지아가 다시 고개를 들었다.",
  ].join(" ");
  const contradiction = findScenePresenceContradiction({
    text,
    currentUserText: "누가 먼저 대답해줄래?",
    presentCharacters: present,
  });

  assert.deepEqual(present.map((fact) => fact.characterName), ["박지아", "소은"]);
  assert.equal(contradiction?.kind, "unauthorized_exit");
  assert.match(contradiction?.matchedText || "", /자취를 감췄/u);
});

test("repairs an anonymous exit in place and preserves the valid roster afterward", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      {
        role: "assistant",
  ...
  const result = removeScenePresenceContradictionPassages({
    text: [
      "*곁에서 숨을 죽이고 있던 다른 형체는 그림자 속으로 자취를 감췄다. 오직 소은만 홀로 남겨졌다.*",
      "",
      "*소은이 먼저 대답하자 지아도 곁에서 고개를 들었다.*",
      "",
      "```상태",
      "장소: 거실",
      "```",
  ...
  assert.equal(result.removed, 1);
  assert.doesNotMatch(result.text, /자취를 감췄|홀로 남겨졌다/u);
  assert.match(result.text, /박지아, 소은 역시 자리를 벗어나지 않고/u);
  assert.match(result.text, /지아도 곁에서 고개를 들었다/u);
  assert.match(result.text, /```상태\n장소: 거실\n```/u);
});
...
test("repairs a named invented exit without truncating later valid prose", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      { role: "assistant", content: "*박지아와 소은은 창고 안에 함께 남아 있었다.*" },
    ],
    identities: [
      { name: "박지아", aliases: ["지아"] },
      { name: "소은", aliases: [] },
  ...
  const result = removeScenePresenceContradictionPassages({
    text: [
      "*박지아는 공포를 견디지 못하고 창고 밖으로 도망쳤다.*",
      "",
      "*박지아와 소은은 경찰관을 바라보며 떨리는 숨을 골랐다.*",
      "",
      '"저희는 여기 있어요."',
      "",
      "```상태",
    ...
    currentUserText: "둘의 반응을 계속 보여줘.",
    presentCharacters: present,
  });

  assert.equal(result.removed, 1);
  assert.deepEqual(result.kinds, ["unauthorized_exit"]);
  assert.doesNotMatch(result.text, /도망쳤/u);
  assert.match(result.text, /박지아 역시 자리를 벗어나지 않고/u);
  assert.match(result.text, /박지아와 소은은 경찰관을 바라보며/u);
  assert.match(result.text, /저희는 여기 있어요/u);
  assert.match(result.text, /```상태\n장소: 창고\n```/u);
});

test("catches a present character being incapacitated without user authority", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      { role: "assistant", content: "*박지아와 소은은 함께 거실에 남아 있었다.*" },
    ],
    identities: [
      { name: "박지아", aliases: ["지아"] },
      { name: "소은", aliases: [] },
  ...
  const contradiction = findScenePresenceContradiction({
    text: "*극도의 수치심과 통증을 견디지 못한 지아는 헛구역질을 터뜨리며 거실 바닥에 쓰러져 의식을 잃고 말았다.*",
    currentUserText: "지아도 대답해봐.",
    presentCharacters: present,
  });

  assert.equal(contradiction?.characterName, "박지아");
  assert.equal(contradiction?.kind, "unauthorized_incapacitation");
  assert.match(contradiction?.matchedText || "", /지아.*의식을 잃/u);
});

test("repairs invented incapacitation in place and preserves later valid prose", () => {
  const present = deriveCurrentScenePresence({
    messages: [
      { role: "assistant", content: "*박지아와 소은은 함께 거실에 남아 있었다.*" },
    ],
  ...
  const result = removeScenePresenceContradictionPassages({
    text: [
      "*지아는 수치심을 견디지 못하고 그대로 의식을 잃고 말았다.*",
      "",
      "*지아는 거실에 남아 떨리는 손을 움켜쥐었다.*",
      "",
      '"그래도 여기서 대답할게."',
    ].join("\n"),
    currentUserText: "지아도 대답해봐.",
  ...
  assert.equal(result.removed, 1);
  assert.deepEqual(result.kinds, ["unauthorized_incapacitation"]);
  assert.doesNotMatch(result.text, /의식을 잃/u);
  assert.match(result.text, /박지아 역시 깨어 있는 채/u);
  assert.match(result.text, /거실에 남아/u);
  assert.match(result.text, /대답할게/u);
});

test("allows incapacitation when the user explicitly establishes it", () => {
  const present = deriveCurrentScenePresence({
    messages: [{ role: "assistant", content: "*박지아는 거실에 서 있었다.*" }],
    identities: [{ name: "박지아", aliases: ["지아"] }],
  });
  const contradiction = findScenePresenceContradiction({
    text: "*박지아는 충격을 이기지 못하고 의식을 잃고 말았다.*",
    currentUserText: "*지아가 의식을 잃었다*",
    presentCharacters: present,
  });
